...
```
/docs-lint                # 默认：自动检测范围（分支模式或增量模式）
/docs-lint --full         # 全量 lint
```

**范围检测**：
- **分支模式**（当前分支 != main）：通过 `git diff --name-only main...HEAD` 找出分支上的变更，仅 lint 变更涉及的文档和代码
- **增量模式**（当前分支 == main）：从 log.md 中读取上次 lint 记录的 commit ID，仅 lint 自该 commit 以来的变更。首次运行则全量 lint
- `--full`：强制全量 lint

**流程**：

1. 读取 `docs/schema.md` 获取当前约定
2. 确定 lint 范围（分支/增量/全量）
3. **文档内部一致性检查**（范围内）
   a. `[[wiki-links]]` 是否都指向存在的页面
   b. wiki 页面的 `sources` 引用是否指向存在的 raw 文件
   c. 是否有孤立的 wiki 页面
   d. wiki 页面之间是否存在矛盾描述
4. **文档-代码深度审计**（范围内）
   a. 读取变更涉及的代码结构（API 端点、数据模型、配置项、函数签名）
   b. 与 wiki 中的描述逐项对照
   c. 识别差异（文档描述了不存在的东西 / 代码未被文档覆盖 / 文档与代码矛盾）
5. **输出 lint 报告**（🔴 错误 / 🟡 警告 / 🟢 通过）
6. 询问用户是否自动修复（更新 wiki 页面）
7. 追加 `docs/log.md`（含 `scope`、`commit` 字段，commit 作为下次增量 lint 的起点）+ git commit
